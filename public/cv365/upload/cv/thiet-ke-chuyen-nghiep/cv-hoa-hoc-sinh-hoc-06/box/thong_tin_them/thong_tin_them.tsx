...
const Thong_tin_them = ({
	...
	const [subtitle, setSubtitle] = useState<any>(htmldefault?.experiences[4]?.content?.content[0]?.subtitle)
	useEffect(() => {
						...
						<div key={index} id={`exp${index}`} className="ctbx experience">
							...
							<p className="h3">
								<span
									...
									onInput={(e: any) => {
										handleSkillsChange(e, 4, 'subtitle', index);
										handleChangestatusEx(4);
									}}
